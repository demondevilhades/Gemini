package io.github.demondevilhades.gemini.util;
...
public class HttpClient implements Closeable {
    ...
    protected CloseableHttpResponse streamHttp(HttpRequestBase httpRequestBase, Map<String, String> headerMap)
            throws IOException {
        if (headerMap != null) {
            for (Map.Entry<String, String> entry : headerMap.entrySet()) {
                httpRequestBase.addHeader(entry.getKey(), entry.getValue());
        ...
        CloseableHttpResponse closeableHttpResponse = closeableHttpClient.execute(httpRequestBase, httpClientContext);
        int statusCode = closeableHttpResponse.getStatusLine().getStatusCode();
        log.info("stream response: statusCode = {}", statusCode);
